    ...
    try:
        ...
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            ...
            for segment in market_segments:
               # Получаем список точек входа
               file_names  = await entry_point(segment,base_dir,path_market)
               print("Файлы в папке 'ep' (без расширения):", file_names)
...
async def entry_point(segment, base_dir, path_market):
    ...
    if os.path.exists(full_segment_path) and os.path.isdir(full_segment_path):
        ...
        return entry_point, period_values_by_file
    else:
        print(f"Папка '{segment}' не существует: {full_segment_path}")
        return {}, {}


# функция получения всех ref period в рамках одной точки входа
async def get_period_from_rend(path_tab, entry_point):
    for key, value_list in entry_point.items():
        print(f"Ключ: {key}")
            ...
            try:
                ...
                for ref in linkbase_refs:
                    ...
                    if href:
                        ...
                        formula_instant_values = await formula_instant(absolute_paths, entry_point)
...
async def formula_instant (absolute_paths,entry_point):
    values = []
