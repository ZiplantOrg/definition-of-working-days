...
def generator(auto_data_object):
    for month in range(1, 13):
        # идем в обьект
        entry_point(month,auto_data_object)
...
def entry_point (month,auto_data_object):
    # создан класс и перебираем в нем обьекты
    if not data_object:
        #получем обьект
        for entry in auto_data_object:
            # Извлекаем значения из кортежа
            entry_point_val = entry[0]
            refPeriodEnd_val = entry[1]
            refPeriodStart_val = entry[2]
            CurrentPeriodEnd_val = entry[3]

            # Определяем остальные значения по индексу, если они есть
            CurrentPeriodStart_val = entry[4] if len(entry) > 4 else None
            startRepYear_val = entry[5] if len(entry) > 5 else None
            period_val = entry[-1]  # Последний элемент — временной период

            # Создаем объект Entry с правильными аргументами
            obj = Entry(
                entry_point=entry_point_val,
                period=period_val,
                refPeriodEnd=refPeriodEnd_val,
                refPeriodStart=refPeriodStart_val,
                CurrentPeriodEnd=CurrentPeriodEnd_val,
                CurrentPeriodStart=CurrentPeriodStart_val,
                startRepYear=startRepYear_val
            )
